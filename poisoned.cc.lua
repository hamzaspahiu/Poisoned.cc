...
local function autospam()
    if Poisoned.cc.AutoShoot and LocalPlayer.Character:FindFirstChildWhichIsA("Tool") then
...
RunService.Heartbeat:Connect(LPH_JIT(function()
    if Poisoned.cc.Camera and TargetPlr and TargetPlr.Character and Poisoned.cc.SelectedPart then
        if TargetPlr.Character[Poisoned.cc.SelectedPart] then



            Poisoned.cc.velocity = Poisoned.cc.CamResolverEnabled 
                and Recalculate(TargetPlr.Character) 
                or TargetPlr.Character[Poisoned.cc.SelectedPart].AssemblyLinearVelocity

            Poisoned.cc.targetPosition = Poisoned.cc.UseExternal 
                and Vector3.new(
                    TargetPlr.Character[Poisoned.cc.SelectedPart].Position.X + (Poisoned.cc.velocity.X / Poisoned.cc.CamPrediction1),
                    TargetPlr.Character[Poisoned.cc.SelectedPart].Position.Y + (Poisoned.cc.velocity.Y / Poisoned.cc.CamPrediction2),
                    TargetPlr.Character[Poisoned.cc.SelectedPart].Position.Z + (Poisoned.cc.velocity.Z / Poisoned.cc.CamPrediction1)
                )
                or Vector3.new(
                    TargetPlr.Character[Poisoned.cc.SelectedPart].Position.X + (Poisoned.cc.velocity.X * Poisoned.cc.CamPrediction1),
                    TargetPlr.Character[Poisoned.cc.SelectedPart].Position.Y + (Poisoned.cc.velocity.Y * Poisoned.cc.CamPrediction2),
...
if Poisoned.cc.CamWallCheck and BehindWall(TargetPlr) then
                return
            end

            if Poisoned.cc.CAMKo and isPlayerKO(TargetPlr) then
                return
            end

            Camera.CFrame = Camera.CFrame:Lerp(
                CFrame.new(Camera.CFrame.Position, Poisoned.cc.targetPosition), 
                Poisoned.cc.smoothness or 0.1, 
                Enum.EasingStyle[Poisoned.cc.easingStyle], 
                Enum.EasingDirection[Poisoned.cc.easingDirection]
            )
        end
    end
end))

LocalPlayer.Character:WaitForChild("Humanoid").Staccanged:Connect(function(old, new)
    if Poisoned.cc.JumpBreak and new == Enum.HumanoidStateType.Freefall then
        task.wait(0.17)
...
local tool_activated = function(character)
    character.ChildAdded:Connect(function(child)
        if Poisoned.cc.bool_at_tp then
...
RunService.Heartbeat:Connect(LPH_JIT(function()
 ...
 if Poisoned.cc.cframespeedtoggle and game.Players.LocalPlayer.Character:FindFirstChild("Humanoid") then
        local humanoidRootPart = game.Players.LocalPlayer.Character.HumanoidRootPart
        local moveDirection = game.Players.LocalPlayer.Character.Humanoid.MoveDirection
        
        humanoidRootPart.CFrame = humanoidRootPart.CFrame + moveDirection * (Poisoned.cc.speedvalue / 0.5)
    end
end)) 
...
if Poisoned.cc.FriendCheck and player:IsFriendsWith(Plr.UserId) then
                continue
            end

if Poisoned.cc.TeamCheck and isPlayerOnSameTeam(Plr) then
    continue
end

            if Poisoned.cc.KOCheck and isPlayerKO(Plr) then
                continue
            end

            if Poisoned.cc.SeatedCheck and isPlayerSeated(Plr) then
                continue
            end

            if Poisoned.cc.WallCheck and BehindWall(Plr) then
                continue
            end
...
RunService.Heartbeat:Connect(LPH_JIT(function()
    if Poisoned.cc.SilentMode then
...
local Window = Library:Window({
    Name = 'Psalms.<font color="'..RealColor..'">cc</font> | '..guh..' | '..dick,
    Amount = 4
})
...
local Watermark = Library:Watermark({Name = string.format("🤑 Poisoned.cc 🤑")})
...
Flag = "Auto Air",
    Callback = function(a)
        Poisoned.cc.AutoAir = a
    end
})
TargetAimSection:Textbox({
    Name = "Auto Air Delay",
Flag = "Auto Air Delay",
  Default = tostring(Poisoned.cc.ShootDelay),
    Callback = function(a)
...
Flag = "Locking Method",
    ...
    Default = Poisoned.cc.LockType,
    Callback = function(a)
        Poisoned.cc.LockType = a
    end
})
...
Flag = "Horizontal Prediction X",
    Default = 0.1,
    Callback = function(a)
        Poisoned.cc.HorizontalPrediction2 = tonumber(a) or 0.1
task.wait(0.01)
Poisoned.cc.HorizontalPrediction = Poisoned.cc.HorizontalPrediction2
    Poisoned.cc.VerticalPrediction = Poisoned.cc.VerticalPrediction2
    end
})
PredictionSection:Textbox({
...
Flag = "Vertical Prediction Y",
    Default = 0.1,
    Callback = function(a)
        Poisoned.cc.VerticalPrediction2 = tonumber(a) or 0.1
task.wait(0.01)
Poisoned.cc.HorizontalPrediction = Poisoned.cc.HorizontalPrediction2
    Poisoned.cc.VerticalPrediction = Poisoned.cc.VerticalPrediction2
    end
...
PredictionSection:Textbox({
    Name = "Jump offset",
Flag = "JumpOffset",
    Default = Poisoned.cc.jumpoffset2,
    Callback = function(a)
        Poisoned.cc.jumpoffset2 = tonumber(a)
    end
})
PredictionSection:Textbox({
    Name = "Fall offset",
Flag = "FallOffset",
    Default = Poisoned.cc.jumpoffset3,
    Callback = function(a)
        Poisoned.cc.jumpoffset3 = tonumber(a)
    end
})
PredictionSection:Toggle({
    Name = "Visualize",
...
Flag = "Auto Prediction Mode",
    ...
    Default = Poisoned.cc.APMODE,
    Callback = function(a)
        Poisoned.cc.APMODE = a
    end
})
PredictionSection:List({
...
DisableShiftlock()
 ...
 if a then
   ...
   humanoidplayemote(Vars.BackpackData.Humanoid, MacroEmote[Poisoned.cc.MacroDance])
    task.wait(Poisoned.cc.MacroDanceDelay)
...
RunService.RenderStepped:Connect(function(DeltaTime)
    ...
    if TimeElapsed >= Poisoned.cc.MacroSpeed then
...
MTSectionS:Textbox({
    Name = "Latency",
Flag = "Macro Latency",
    Default = tostring(Poisoned.cc.MacroDanceDelay),
    Callback = function(a)
        Poisoned.cc.MacroDanceDelay = tonumber(a)
    end
})
...
MTSectionS:Textbox({
    Name = "Speed",
Flag = "Macro Speed",
    Default = tostring(Poisoned.cc.MacroSpeed),
    Callback = function(a)
        Poisoned.cc.MacroSpeed = tonumber(a)
    end
})
...
if dog then
    ...
    humanoid.WalkSpeed = Poisoned.cc.MacroSpeed * 1000
    ...
    for _, v in pairs(game.Players:GetPlayers()) do
        if v ~= player and v.Character and v.Character:FindFirstChild("HumanoidRootPart") 
           and v.Character.Humanoid.Health > 0 then
            if Poisoned.cc.FriendCheck and player:IsFriendsWith(v.UserId) then
                continue
            end

if Poisoned.cc.TeamCheck and isPlayerOnSameTeam(v) then
    continue
end

            if Poisoned.cc.KOCheck and isPlayerKO(v) then
                continue
            end

            if Poisoned.cc.SeatedCheck and isPlayerSeated(v) then
                continue
            end

            if Poisoned.cc.WallCheck and BehindWall(v) then
                continue
            end
...
RunService.Heartbeat:Connect(function(DeltaTime)
    ...
    if PussyTime >= 0.1 and TargBindEnabled then
        if TargetPlr and TargetPlr.Character and TargetPlr.Character:FindFirstChild("Humanoid") then
            if Poisoned.cc.UnlockOnKO and isPlayerKO(TargetPlr)  then
                toggleLock()
                return
            end
        end
...
Flag = "Cam Resolver",
    Callback = function(a)
        Poisoned.cc.CamResolverEnabled = a
    end
})
Bumt:Textbox({
    Name = "Camera Smoothness",
Flag = "Cam Smoothness",
    Default = tostring(Poisoned.cc.smoothness),
    Callback = function(a)
        Poisoned.cc.smoothness = tonumber(a)
    end
})
...
Bumt:List({
    ...
    Default = Poisoned.cc.easingStyle,
    Callback = function(a)
        Poisoned.cc.easingStyle = a
    end
})
...
Bumt:List({
    ...
    Default = Poisoned.cc.easingDirection,
    Callback = function(a)
        Poisoned.cc.easingDirection = a
    end
})
...
Flag = "Cam HPred",
    ...
    Callback = function(a)
        camgay2 = tonumber(a)
task.wait(0.01)
Poisoned.cc.CamPrediction2 = camgay
      Poisoned.cc.CamPrediction1 = camgay2
    end
})
CamPred:Textbox({
...
Flag = "Auto Prediction",
    Callback = function(a)
        Poisoned.cc.CamAutoprediction = a
       Poisoned.cc.CamPrediction2 = camgay
      Poisoned.cc.CamPrediction1 = camgay2
    end
})
...
Dog11:Button({
    ...
    Callback = function()
        ...
        MakeButton("Speed", Color3.fromRGB(155, 0, 155), function(state)
            Poisoned.cc.cframespeedtoggle = state
        end)
    end
})
